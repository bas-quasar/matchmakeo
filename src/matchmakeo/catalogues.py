...
class Catalogue(ABC):
    ...
    def _create_table(self, connection:Connection, product:Product, primary_key:str='pk'):
        ...
        metadata = MetaData()
        table = Table(product.table_name, metadata,
        ...
        if not dry_run:
            ...
            for granule in granules:
                insertion = table.insert().values(
                    ...
                    datetime_start=datetime.strptime(granule[1]['time_start'], "%Y-%m-%dT%H:%M:%S.%fZ"), # using datetime.strptime instead of datetime.fromisoformat for back compatibility pre-python 3.11
                    datetime_end=datetime.strptime(granule[1]['time_end'], "%Y-%m-%dT%H:%M:%S.%fZ"),
                    **granule[1],
                    )
        ...
        if not dry_run:
            ...
            for prod in search_results.products():
                ...
                insertion = table.insert().values(
                    ...
                    datetime_start=datetime.strptime(prod.properties['beginPosition'], "%Y-%m-%dT%H:%M:%S.%fZ"), # using datetime.strptime instead of datetime.fromisoformat for back compatibility pre-python 3.11
                    datetime_end=datetime.strptime(prod.properties['endPosition'], "%Y-%m-%dT%H:%M:%S.%fZ"),
