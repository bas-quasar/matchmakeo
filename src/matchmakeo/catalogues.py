...
class Catalogue(ABC):
    ...
    def _check_database_dryrun(self, database: Database, dry_run: bool):
        ...
        if not dry_run and not Database:
            warnings.warn(
                f"Got dry_run: {dry_run} and database: {database}. Database needed unless dry_run=True."
            )
...
class NasaCMR(Catalogue):
    ...
    def __init__(
        ...
        if client_id is None:
            log.warning(
                "No client_id set. Client ids are strongly encouraged by NASA CMR, we suggest using your name or research group's name, for example."
            )
...
class EarthEngine(Catalogue):
    ...
    def __init__(
        ...
        if not ee:
            raise ImportError(
                "Earth Engine Catalogue interface requires the earthengine-api package, install with `pip install matchmakeo[earthengine]` or `pip install earthengine-api`."
            )

        if not project_id:
            raise ValueError(f"Earth Engine requires a project name. Got {project_id=}")

        self.initialise_earth_engine(
    ...
    def download_footprints(
        ...
        def fetch_day_metadata(day_string):
            ...
            day_collection = (
                ee.ImageCollection(product.name)
            ...
            for date_str, granules in results:
                ...
                if dry_run:
                    ...
                    for granule in granules:
                        insertion = table.insert().values(
                            # id=granule['id'],
                            geometry=coords_to_polygon(
                                granule["properties"]["system:footprint"]["coordinates"]
                            ),
...
class JaxaGportal(Catalogue):
    ...
    def __init__(
        ...
        if username or password is None:
            log.error(
                "Got None for either username or password, these must be passed as arguments or set via the environment variables GPORTAL_USERNAME and GPORTAL_PASSWORD."
            )
            ...
            for prod in search_results.products():
                ...
                insertion = table.insert().values(
                    ...
                    **{
                        ...
                        if k
                        not in [
                            f.catalogue_name for f in self.fields + product.extra_fields
                        ]
                    },
                )
                connection.execute(insertion)
